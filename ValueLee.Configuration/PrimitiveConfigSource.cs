...
namespace ValueLee.Configuration
{
    public class PrimitiveConfigSource : IDisposable
    {
        ...
        private readonly Dictionary<string, string> _innerConfigs = new Dictionary<string, string>();
        ...
        private PrimitiveConfigSource(string xmlString, string xmlDocumentPath)
        {
            ...
            // handle the inner configs
            var innerConfigs = xmlDocument.SelectNodes("root/innerconfig");
            foreach (XmlNode innerConfig in innerConfigs)
            {
                if (innerConfig.Attributes["name"] == null)
                {
                    throw new ConfigException($"The 'name' attribute is required. Hint:'{innerConfig.OuterXml}'");
                }
                if (innerConfig.Attributes["value"] == null)
                {
                    throw new ConfigException($"The 'value' attribute is required. Hint:'{innerConfig.OuterXml}'");
                }
                var name = innerConfig.Attributes["name"].Value.Trim();
                var value = innerConfig.Attributes["value"].Value.Trim();
                if (_innerConfigs.ContainsKey(name))
                {
                    throw new ConfigException($"The 'name' attribute must be unique. Hint:'{innerConfig.OuterXml}'");
                }
                _innerConfigs.Add(name, value);
            }
        ...
        private void Travel(ConfigNode configNode)
        {
            foreach (var item in configNode.ConfigItems)
            {
                ...
                var options = CustomizeConfigItemOptionsSource(configPath);
...
#elif NET462_OR_GREATER || NET6_0_OR_GREATER
                _channel.Writer.WriteAsync(needModifyinglist);
#endif
        ...
        protected virtual string[] CustomizeConfigItemOptionsSource(string configItem)
        {
            return null;
        }

        protected virtual Func<string, bool> CustomizeConfigItemValidationRule(string configItem)
        {
            return null;
        }
        ...
        private void ValidateValue(string configItem, object value)
        {
            ...
            var validationRule = CustomizeConfigItemValidationRule(configItem);
